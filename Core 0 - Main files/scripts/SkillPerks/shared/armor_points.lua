...
--[[
    ...
    Beast races cannot wear boots, so full-set checks treat Boots as required
    only for actors whose race can use that slot.
]]
...
--- @param actor table
--- @return boolean True if this actor's race can use the boot slot.
function ArmorPoints.actorCanWearBoots(actor)
    if not actor or not types.NPC.objectIsInstance(actor) then
        return true
    end

    local okNpc, npcRecord = pcall(types.NPC.record, actor)
    if not okNpc then
        return true
    end

    local raceId = npcRecord and npcRecord.race
    if not raceId then
        return true
    end

    local okRace, raceRecord = pcall(types.NPC.races.record, raceId)
    if not okRace then
        return true
    end

    return not (raceRecord and raceRecord.isBeast)
end

--- Builds the armor slots that count for a full set on this actor.
--- Beast races cannot equip boots, so the boot slot is omitted for them.
--- @param actor table
--- @return table Slot names from TRACKED_SLOTS.
function ArmorPoints.getRequiredFullSetSlots(actor)
    local requiredSlots = {
        "Cuirass", "Greaves", "LeftPauldron", "RightPauldron",
        "LeftGauntlet", "RightGauntlet",
    }

    if ArmorPoints.actorCanWearBoots(actor) then
        table.insert(requiredSlots, "Boots")
    end
...
--- Heavy Armor D chain's "full set" check. Requires all wearable body armor
--- slots to be Heavy Armor. Boots are skipped for beast races, while helmet
--- and shield remain excluded.
--- @param actor table
--- @return boolean
function ArmorPoints.isFullHeavySet(actor)
    local _, breakdown = ArmorPoints.getPoints(actor)
    for _, slotName in ipairs(ArmorPoints.getRequiredFullSetSlots(actor)) do
