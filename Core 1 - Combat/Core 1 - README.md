...
- Terror of the Fallen's chance is `20% + the victim's Health percentage
  ...
  nearby actor other than the player and slain victim, including allies.
...
- `luaheavyarmor debug`
- `lualongblade debug`
- `luamediumarmor debug`
- `luaspear debug`

Each command reports the skill's owned chain ranks and the live state used by
its effects, such as trigger queues, cooldowns, equipment checks, stacks, and
the most recent observed hit where applicable.

Append ` trace` to any command to toggle its live activation trace, such as
`luablunt debug trace`. Trace output requires SkillPerks verbosity `3`; enter
the same command again to disable it.

Shorter aliases remain available: `luaarm debug`, `luaath debug`, `luabl debug`,
`luaha debug`, `lualb debug`, and `luama debug`.
